...
\textbf{Satz zur Viskosität}

\section{Theorie}
\label{sec:theorie}
\subsection{Kapillarität}
Zwischen Molekülen wirken \textit{Dipolkräfte} und \textit{Van-der-Waals-Kräfte}. WARUM??? Die in diesem Versuch verwendeten Flüssigkeiten\\
BILD\\
An der Oberfläche einer Flüssigkeit wirken also nur die Kräfte \textbf{rein} in die Flüssigkeit, während in der Flüssigkeit auf ein Molekül Kräfte von allen Seiten wirken und sich so ausgleichen.\\
...
Wenn man das Volumen des Probekörpers nicht kennt, muss zuerst Wasser als Referenz genommen werden ($\rho_{H_2O}\approx 1~ \si{\gram/\centi \meter ^3}$).\\
FORMEL???
...
\end{align}
Setzt man diese Kraft gleich der Druckkraft $F_p=\pi r^2 \cdot (p_1-p_2)$ kann das \textit{Hagen-Poiseuille Gesetz} der laminaren Rohrströmung hergeleitet werden.\footnote{\cite{gerthsen} S.125} % hier muss noch ergänzt werden
\begin{align}
...
Zuerst werden die Durchmesser der 3 verschiedenen Kapillaren jeweils dreimal mit dem Messmikroskop vermessen. Dabei wird die Kapillare fokussiert, dann der linke oder rechte Rand anvisiert. Die Stellung des ??? wird abgelesen, bevor man die gegenüberliegende Seite anvisiert und erneut die ??? abliest. Aus der Differenz der beiden ergibt sich der Durchmesser der Kapillare.
...
Anschließend werden Methanol und Ethylenglykol in spezielle Behälter gegossen und somit vorschriftsmäßig entsorgt. Nun müssen alle Gefäße und die Kapillaren gereinigt werden.
